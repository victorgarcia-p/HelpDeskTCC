$(document).ready(function () {
    ...
    $(document).on('click', '#editInfo', function (e) {
        ...
        $('#idCategoria').val(currentRow.find("td:eq(0)").text());
...
function CarregarTabelaCategoria() {
...
function CadastrarCategoria(categoria) {
    ...
    $.ajax({
        url: '/gerenciar/cadastrarcategoria',
        type: "POST",
...
function EditarCategoria(categoria) {
    $('#modalCarregando').modal('show');
    $.ajax({
        url: '/gerenciar/atualizarcategoria',
        type: "POST",
        data: { categoria: categoria},
        success: function (result) {
            $('#modalCadastro').modal('hide');
...
function ExcluirCategoria(categoria) {
    $('#modalCarregando').modal('show');
    $.ajax({
        url: '/gerenciar/excluircategoria',
        type: "DELETE",
        data: { categoria: categoria },
        success: function (result) {
            alert(result);
